...
import { tpDocTypesTree } from '../dist/tree.js';
...
for await (const [index, tpKey] of Object.keys(masterIds).entries()) {
  ...
  try {
    ...
    await ensureResource(trellisPath, trellisResult, 'application/vnd.trellis.1+json');
    await oada.delete({
    ...
    await ensureResource(docsPath, docs, 'application/vnd.trellis.documents.1+json');
    await oada.delete({
  ...
  //      documentType === 'documents'
      ...
      if (documentType === 'documents') {
        ...
        if (docTypeLink._meta && !isLink(docTypeLink._meta) && Object.keys(docTypeLink).length === 1) {
      ...
      await fixListLibEntry(docTypePath, tpKey, documentType);
    }
  } catch (err) {
    console.log(err)
  }
...
*/
async function fixListLibEntry(path, tpKey, docType, list) {
  try {
    let { data: listLibEntry } =  await oada.get({
      path: `${path}/_meta/oada-list-lib`
    })

    let newEntry = JSON.parse(JSON.stringify(listLibEntry));
    let newEntryName = `lf-sync:to-lf:${tpKey}:${docType}`;
    let wierdMasterid = `${tpKey}:`;

    // Find old list lib entry using new style masterids
    let hasMasterid = listLibEntry && listLibEntry['lf-sync:to-lf:'] && listLibEntry['lf-sync:to-lf:'][wierdMasterid];
    let hasMasteridDocType = hasMasterid && listLibEntry['lf-sync:to-lf:'][wierdMasterid][docType];

    // Find old list lib entry using old style masterids
    let hasOldMasterid = listLibEntry && listLibEntry['lf-sync:to-lf:'] && Object.keys(listLibEntry['lf-sync:to-lf:']).some(key => key !== wierdMasterid)
    let oldMasterids = (hasOldMasterid && Object.keys(listLibEntry['lf-sync:to-lf:']).filter(key => key !== wierdMasterid)) || [];
    let oldMasterid = hasOldMasterid && Object.keys(listLibEntry['lf-sync:to-lf:']).find(key => key !== wierdMasterid);
    if (oldMasterids.length > 1) {
      console.log('uh oh, multiple old masterids found here...');
    }
    let hasOldMasteridDocType = hasOldMasterid && listLibEntry['lf-sync:to-lf:'][oldMasterid][docType];

    // Find current list lib entry using
    let hasNewEntry = hasMasterid && listLibEntry[newEntryName]

    if (hasNewEntry) {
      newEntry[newEntryName] = listLibEntry[newEntryName];
    } else if (hasMasteridDocType) {
      newEntry[newEntryName] = listLibEntry['lf-sync:to-lf:'][wierdMasterid][docType];
    } else if (hasOldMasteridDocType) {
      newEntry[newEntryName] = listLibEntry['lf-sync:to-lf:'][oldMasterid][docType];
    }
    delete newEntry['lf-sync:to-lf:'];

    console.log(`The newEntry for path ${path}: ${JSON.stringify(newEntry, 0,2)}`);
...
function isLink(obj) {
  ...
  return (keys.length === 1 && obj._id) || (keys.length == 2 && obj._id && obj._rev);
}
...
async function ensureResource(path, data, type) {
  if (!isResource(data)) {
    ...
    let contentType = contentTypes[type];

    if (!contentType) {
      console.log(`Oh no, couldnt find contentType for ${type}. Came up here at ${path}`);
      return;
    }
    if (data._meta) delete data._meta;

    try {
      let response = await oada.post({
        path: '/resources',
        contentType: contentType,
        data
      })
      let _id = response.headers['content-location'].substring(1);
...
const contentTypes = {
  ...
  'letters-of-guarantee': 'application/vnd.trellisfw.letter-of-guarantee.1+json',
  'emergency-contact-information': 'application/vnd.trellisfw.emergency-contact-information.1+json',
  'sars': 'application.vnd.trellisfw.sars.1+json',
  'w-9s': 'application/vnd.trellisfw.w-9.1+json',
  'nutritional-information': 'application/vnd.trellisfw.nutritional-information.1+json',
  'allergen-statements': 'application/vnd.trellisfw.allergen-statement.1+json',
  'be-ingredient-statements': 'application/vnd.trellisfw.be-ingredient-statement.1+json',
  'ingredient-statements': 'application/vnd.trellisfw.ingredient-statement.1+json',
  'ca-prop-65-statements': 'application/vnd.trellisfw.ca-prop-65-statement.1+json',
  'coo-statements': 'application/vnd.trellisfw.coo-statement.1+json',
  'gluten-statements': 'application/vnd.trellisfw.gluten-claim-statement.1+json',
  'ingredient-breakdowns': 'application/vnd.trellisfw.ingredient-breakdown.1+json',
  ...
  'srm-statements-audits': 'application/vnd.trellisfw.srm-statement-audit.1+json',
  'srm-audits': 'application/vnd.trellisfw.srm-audit.1+json',
  'srm-statements': 'application/vnd.trellisfw.srm-statement.1+json',
  'srm-corrective-actions': 'application/vnd.trellisfw.srm-corrective-actions.1+json',
  'ecoli-audits': 'application/vnd.trellisfw.ecoli-audit.1+json',
  'foreign-material-control-plans': 'application/vnd.trellisfw.foreign-material-control-plan.1+json',
  'animal-welfare-audits': 'application/vnd.trellisfw.animal-welfare-audit.1+json',
  'humane-harvest-statements': 'application/vnd.trellisfw.humane-harvest-statement.1+json',
  'nrp-statements': 'application/vnd.trellisfw.nrp-statement.1+json',
  'lot-code-explanations': 'application/vnd.trellisfw.lot-code-explanation.1+json',
  ...
  'copacker-fsqa-questionnaires': 'application/vnd.trellisfw.copacker-fsqa-questionnaire.1+json',
  'copack-confidentiality-agreement-forms': 'application/vnd.trellisfw.copack-confidentiality-agreement-form.1+json',
  'tpa-corrective-actions': 'application/vnd.trellisfw.tpa-corrective-actions.1+json',
  'tpa-food-safety-audits': 'application/vnd.trellisfw.tpa-food-safety-audit.1+json',
  'tpa-animal-welfare-audits': 'application/vnd.trellisfw.tpa-animal-welfare-audit.1+json',
  'tpa-animal-welfare-corrective-actions': 'application/vnd.trellisfw.tpa-animal-welfare-corrective-actions.1+json',
  'w-8s': 'application/vnd.trellisfw.w-8.1+json',
  'animal-welfare-corrective-actions': 'application/vnd.trellisfw.animal-welfare-corrective-actions.1+json',
  'fsqa-certificates': 'application/vnd.trellisfw.fsqa-certificate.1+json',
  'signed-vendor-acknowledgement-forms': 'application/vnd.trellisfw.signed-vendor-acknowledgement-form.1+json',
  ...
  'ecoli-intervention-statements-audits': 'application/vnd.trellisfw.ecoli-intervention-statement-audits.1+json',
